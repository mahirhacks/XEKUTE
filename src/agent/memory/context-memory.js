const ContextMemory = (() => {
  const InitialPrompts = typeof module !== "undefined" && module.exports
    ? require("../../prompts/instructions/initial-context")
  ...
  function oneLine(value, maxChars = 1000) {
    ...
    const sampleLimit = Math.max(maxChars * 6, maxChars + 2048);
    const sampled = raw.length > sampleLimit
      ? `${raw.slice(0, Math.floor(sampleLimit * 0.72))}\n... [large payload omitted] ...\n${raw.slice(-Math.ceil(sampleLimit * 0.28))}`
      : raw;
    const text = cleanText(sampled).replace(/\s+/g, " ");
  ...
  function compactToolContent(content) {
    ...
    if (!source || source.length > 64_000) return "";
    let parsed = null;
    try { parsed = JSON.parse(source); } catch { return ""; }
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) return "";
    let payload = parsed.payload && typeof parsed.payload === "object" && !Array.isArray(parsed.payload)
      ? parsed.payload
      : {};
    if (typeof parsed.payload === "string" && parsed.payload.length <= 64_000) {
      try {
        const decoded = JSON.parse(parsed.payload);
        if (decoded && typeof decoded === "object" && !Array.isArray(decoded)) payload = decoded;
      } catch { /* A plain-text payload is represented only by the outer status. */ }
    }
    const boundedRefs = (value) => (Array.isArray(value) ? value : [])
      .slice(0, 50)
      .map((item) => oneLine(item, 240))
      .filter(Boolean);
    const detailKeys = ["command", "shell", "executable", "args", "cwd", "exitCode", "signal", "elapsedMs", "timedOut", "terminationReason", "method", "url", "path", "statusCode", "outputCompleteness"];
    const details = {};
    for (const key of detailKeys) {
      if (payload[key] === undefined || payload[key] === null || payload[key] === "") continue;
      details[key] = Array.isArray(payload[key])
        ? payload[key].slice(0, 24).map((item) => oneLine(item, 240))
        : typeof payload[key] === "boolean" || typeof payload[key] === "number"
          ? payload[key]
          : oneLine(payload[key], 500);
    }
    if (payload.stdout) details.stdoutPreview = oneLine(payload.stdout, 800);
    if (payload.stderr) details.stderrPreview = oneLine(payload.stderr, 500);
    const projected = {
      ...(typeof parsed.ok === "boolean" ? { ok: parsed.ok } : {}),
      ...(parsed.code ? { code: oneLine(parsed.code, 160) } : {}),
      ...(parsed.errorCode ? { errorCode: oneLine(parsed.errorCode, 160) } : {}),
      ...(parsed.status ? { status: oneLine(parsed.status, 240) } : {}),
      ...(parsed.summary ? { summary: oneLine(parsed.summary, 2_000) } : {}),
      ...(parsed.error ? { error: oneLine(parsed.error, 2_000) } : {}),
      evidenceIds: boundedRefs(parsed.evidenceIds),
      evidence_refs: boundedRefs(parsed.evidence_refs),
      payload: {
        evidenceIds: boundedRefs(payload.evidenceIds),
        evidence_refs: boundedRefs(payload.evidence_refs),
      },
  ...
  function normalizedCompactionRecords(messages) {
    const records = [];
    ...
    for (const message of Array.isArray(messages) ? messages : []) {
      ...
      if (role !== "tool") {
        records.push({ message, entry, count: 1, removed: false });
        continue;
      }
      const key = `${String(message?.tool_name || message?.toolName || message?.name || "tool").toLowerCase()}\u0000${entry.toLowerCase()}`;
      const previousIndex = repeatedTools.get(key);
      const count = previousIndex === undefined ? 1 : records[previousIndex].count + 1;
      if (previousIndex !== undefined) records[previousIndex].removed = true;
      records.push({ message, entry, count, removed: false });
      repeatedTools.set(key, records.length - 1);
    }
    return records
      .filter((record) => !record.removed)
      .map((record) => ({
        ...record,
        entry: record.count > 1 ? `${record.entry} [repeated ${record.count} times]` : record.entry,
      }));
  }

  function projectDurableMessages(messages, { maxMessages = 400 } = {}) {
    const limit = Math.max(1, Math.min(Number(maxMessages) || 400, 400));
    return (Array.isArray(messages) ? messages : []).slice(-limit).map((message) => {
      const role = String(message?.role || "message");
      const projected = {
        role,
        id: oneLine(message?.id || "", 240),
        createdAt: message?.createdAt || null,
      };
      if (role === "tool") {
        projected.tool_name = oneLine(message?.tool_name || message?.toolName || message?.name || "tool", 160);
        projected.content = compactToolContent(message?.content);
      } else {
        projected.content = oneLine(message?.content ?? message?.text ?? "", 4_000);
      }
      return projected;
    });
  }

  function projectRecentContextMessages(messages) {
    return (Array.isArray(messages) ? messages : []).map((message) => {
      const role = String(message?.role || "message");
      const projected = message && typeof message === "object" ? { ...message, role } : { role };
      if (role === "tool") {
        const toolName = String(message?.tool_name || message?.toolName || message?.name || "tool");
        projected.content = toolName === "query_knowledge"
          ? JSON.stringify({ status: "knowledge lease expired during context compression" })
          : compactToolContent(message?.content) || oneLine(message?.content, 1_400);
      } else {
        const limit = role === "user" ? 8_000 : 6_000;
        const content = cleanText(message?.content ?? message?.text ?? "");
        projected.content = content.length <= limit
          ? content
          : `${content.slice(0, Math.floor(limit * 0.72)).trimEnd()}\n\n... [older message detail omitted during compression] ...\n\n${content.slice(-Math.ceil(limit * 0.28)).trimStart()}`;
      }
      return projected;
    });
  }
  ...
  function messageEntry(message) {
    ...
    const toolName = String(message?.tool_name || message?.toolName || message?.name || "tool");
    const leasePayload = toolName === "query_knowledge" || toolName.startsWith("mcp__");
    const structuredToolContent = role === "tool" && !leasePayload ? compactToolContent(message?.content) : "";
    const content = leasePayload ? "" : oneLine(structuredToolContent || message?.content, contentLimit);
    ...
    if (role === "tool") {
      const name = toolName;
      if (leasePayload) return `TOOL RESULT (${name}): knowledge lease content omitted from durable context`;
  ...
  function buildMemoryTranscript(previousSummary, messages, { contextTokens = 4096 } = {}) {
    ...
    const entries = normalizedCompactionRecords(messages).map((record) => record.entry);
  ...
  function buildFallbackSummary(previousSummary, messages, { contextTokens = 4096 } = {}) {
    ...
    for (const record of normalizedCompactionRecords(messages)) {
