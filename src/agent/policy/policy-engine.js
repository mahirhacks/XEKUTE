...
} = require("../../prompts/rules/runtime-policy-rules");
...
const PLAN_DOCUMENT_RE = /(?:^|[\\/])(?:plans?|roadmaps?|tasks?)(?:[\\/]|$)|(?:^|[\\/])[^\\/]*(?:plan|roadmap|task)[^\\/]*\.(?:md|markdown|txt|json|ya?ml)$/i;

function isPlanFilePath(value) {
  const normalized = String(value || "").replace(/\\/g, "/").replace(/^\/+/, "");
  return Boolean(normalized && PLAN_DOCUMENT_RE.test(normalized));
}
...
function evaluateAction({ tool, profile: rawProfile, policy = DEFAULT_POLICY, approvalGranted = false } = {}) {
  ...
  const planFile = profile.capability === "plan"
    && toolNameOf(tool) === "create_file"
    && isPlanFilePath(tool?.file || tool?.args?.path);
  ...
  if (profile.capability === "plan" && classification.mutatesWorkspace && !planFile) {
    return { ...result, allowed: false, reason: "Planner mode can only create a clearly named plan document.", code: "PLAN_FILE_ONLY" };
  }
  if (["observe", "assess", "plan", "verify", "report"].includes(profile.capability) && (classification.active || classification.mutatesWorkspace || classification.mutatesAssessment) && !planFile) {
