/* Prompt orchestration and untrusted context packaging. Model text lives in prompts/instructs/. */

const CHAT_MODES = new Set(["agent", "plan", "ask"]);
const DEFAULT_CONTEXT_TOKENS = 8192;
const { normalizeProfile, profileKey } = require("./policy/operating-modes");
...
function resolveTools(tools, context = {}) {
  return tools.map((tool) => {
    ...
    if (!tool.file) {
      const fileActions = ["read_file", "write_file", "create_file", "patch_file", "replace_in_file", "insert_in_file", "append_file", "delete_file", "get_file_outline"];
...
function buildSystemContext({ mode = "agent", modeFamily = "assist", promptConfig = null, depth = "operational", specializedGuidance = "" } = {}) {
