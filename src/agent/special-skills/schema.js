...
function normalizeManifest(metadata = {}, { id = "", source = "", resources = [] } = {}) {
  ...
  const command = string(metadata.command, "", 120).toLowerCase();
...
function validateManifest(manifest) {
  ...
  if (!manifest || !COMMAND_RE.test(manifest.command)) errors.push("command must be an exact slash command such as /pentest");
...
module.exports = Object.freeze({ COMMAND_RE, DEFAULT_MODES, ID_RE, SUPPORTED_MODES, normalizeManifest, validateManifest });
