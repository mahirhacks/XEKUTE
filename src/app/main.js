...
const { createToolHandlers } = require("../harness/core/tool-handlers");
const { createWorkspaceSearch } = require("../harness/os/workspace-search");
const { createAgentTerminalRunner } = require("../harness/os/terminal-runner");
const { createWebResearch } = require("../harness/cyber/web-research");
const { createWebCloneService } = require("../harness/cyber/webclone");
...
const { runAgentTurn } = require("../agent/controller");
const { normalizeProfile } = require("../agent/policy/operating-modes");
const { loadPolicy, evaluateAction } = require("../agent/policy/policy-engine");
const ScopeEngine = require("../domain/assessment/scope-engine");
const AgentVerifier = require("../agent/verification/verifier");
const { captureOllamaStream } = require("../llm/streams/ollama-stream");
const { captureOpenRouterStream, normalizeOpenRouterMessages, openRouterHeaders, openRouterTools } = require("../llm/streams/openrouter-stream");
const { DEFAULT_OPENROUTER_BASE_URL, normalizeProvider, normalizeBaseUrl, buildChatRequest } = require("../llm/providers");
const { appendAgentAction, appendHypothesis } = require("../agent/memory/action-log");
const ContextMemory = require("../agent/memory/context-memory");
const { createChatSessionStore } = require("./services/chat-session-store");
...
const { appIndexPath } = require("./window-manager");
const {
  GUIDANCE_EXTENSIONS,
  GUIDANCE_KINDS,
  ...
  readGuidanceEntry,
} = require("../agent/instructions/custom-guidance");

const APP_ROOT = path.join(__dirname, "..");
const IS_DEV = process.argv.includes("--dev") || process.env.NODE_ENV === "development";
const APP_INDEX_PATH = appIndexPath(path, __dirname);
...
function projectProfileStore() {
  if (!projectProfiles) {
    projectProfiles = createProjectProfileStore({
      ...
      path,
      crypto,
      baseDirectory: path.join(app.getPath("userData"), "project-profiles"),
    });
  }
...
function validateGuidancePath(relativePath) {
  ...
  if (parts.length < 3 || parts[0]?.toLowerCase() !== "custom" || !GUIDANCE_KINDS.includes(kind)) {
    return { error: "Guidance files must live in custom/skills, custom/rules, or custom/instructions", code: "INVALID_GUIDANCE_PATH" };
  }
  ...
  if (!GUIDANCE_EXTENSIONS.has(path.extname(parts[parts.length - 1]).toLowerCase())) {
    return { error: "Guidance files must use .md, .markdown, .txt, .yaml, .yml, or .json", code: "INVALID_GUIDANCE_EXTENSION" };
  }
  return { ok: true, normalized: path.posix.join("custom", kind, ...parts.slice(2)), kind };
}

function resolveGuidanceTarget(workspace, relativePath) {
  const root = path.resolve(String(workspace || ""));
  let rootIsDirectory = false;
  try { rootIsDirectory = Boolean(workspace && path.isAbsolute(root) && fs.existsSync(root) && fs.statSync(root).isDirectory()); } catch { rootIsDirectory = false; }
  if (!rootIsDirectory) {
    return { error: "Open a project before managing custom guidance", code: "WORKSPACE_REQUIRED" };
  }
  ...
  if (!relative || relative.startsWith("..") || path.isAbsolute(relative)) {
    return { error: "Guidance files must stay inside the active project", code: "INVALID_GUIDANCE_PATH" };
  }
  ...
  return { ok: true, root, target, ...validated };
}
...
ipcMain.handle("guidance:entries", async (_event, { workspace } = {}) => {
  const root = guidanceWorkspaceRoot(workspace);
  if (!root) return { error: "Open a project before managing custom guidance", code: "WORKSPACE_REQUIRED" };
  return { ok: true, entries: listWorkspaceGuidance(root) };
});

ipcMain.handle("guidance:read", async (_event, { workspace, relativePath } = {}) => {
  const resolved = resolveGuidanceTarget(workspace, relativePath);
  ...
  const content = readGuidanceEntry(resolved.root, { relativePath: resolved.normalized });
  return content == null ? { error: "Guidance file could not be read", code: "GUIDANCE_READ_FAILED" } : { ok: true, content, relativePath: resolved.normalized, kind: resolved.kind };
});

ipcMain.handle("guidance:context", async (_event, { workspace } = {}) => {
  const root = guidanceWorkspaceRoot(workspace);
  return { ok: true, context: root ? formatWorkspaceGuidance(root) : "" };
});

ipcMain.handle("guidance:save", async (_event, { workspace, relativePath, content = "" } = {}) => {
  const resolved = resolveGuidanceTarget(workspace, relativePath);
  if (resolved.error) return resolved;
  ...
  try {
    ...
    return { ok: true, relativePath: resolved.normalized, kind: resolved.kind };
  } catch (error) {
    return { error: error.message, code: "GUIDANCE_WRITE_FAILED" };
  }
});

ipcMain.handle("guidance:import", async (_event, { workspace, kind } = {}) => {
  const root = guidanceWorkspaceRoot(workspace);
  if (!root) return { error: "Open a project before importing custom guidance", code: "WORKSPACE_REQUIRED" };
  ...
  try {
    for (const source of picked.filePaths.slice(0, 20)) {
      ...
      let candidate = path.posix.join("custom", selectedKind, baseName);
      let target = path.resolve(root, ...candidate.split("/"));
      let suffix = 2;
      while (fs.existsSync(target)) {
        const parsed = path.parse(baseName);
        candidate = path.posix.join("custom", selectedKind, `${parsed.name}-${suffix}${parsed.ext}`);
    ...
    return { ok: true, paths: imported, entries: listWorkspaceGuidance(root) };
  } catch (error) {
    return { error: error.message, code: "GUIDANCE_IMPORT_FAILED" };
  }
});

ipcMain.handle("guidance:delete", async (_event, { workspace, relativePath } = {}) => {
  const resolved = resolveGuidanceTarget(workspace, relativePath);
  if (resolved.error) return resolved;
  try {
    ...
    return { ok: true, relativePath: resolved.normalized };
  } catch (error) {
...
ipcMain.handle("assessment:buildContext", async (_event, { path: assessmentPath } = {}) => {
  ...
  if (picked.canceled || !picked.filePaths.length) return { canceled: true };
  const script = path.join(__dirname, "..", "automation", "context", "parse_context.py");
...
function createAgentTerminalHost(webContents, sendAgentEvent) {
  ...
  return {
    ...
    startProcess: (workspace, command, options = {}) => agentTerminalRunner.startProcess(webContents, workspace, command, { ...options, sendAgentEvent, ownerId: String(webContents.id) }),
...
function runAssessmentIngestProcess(payload = {}) {
  return new Promise((resolve) => {
    const script = path.join(__dirname, "..", "automation", "commands", "assessment_ingest.py");
...
function runSlashCommandPython(action, payload = {}) {
  return new Promise((resolve) => {
    const script = path.join(__dirname, "..", "automation", "commands", "command_parser.py");
    const executable = process.env.POINTER_PYTHON || (process.platform === "win32" ? "python" : "python3");
...
async function listOpenRouterModels() {
  try {
    ...
    return { models: [...new Set(models)].sort(), provider: "openrouter", host: "OpenRouter" };
  } catch (error) {
    return { error: error.message, code: error.code || "OPENROUTER_LIST_FAILED", provider: "openrouter" };
  }
}
async function runOpenRouterChat(event, { messages, model, thinking, tools }, hooks = {}) {
  ...
  try { const request = buildChatRequest({ baseUrl: getOpenRouterBaseUrl(), apiKey: getOpenRouterApiKey(), model: model || llmPreferences().openrouter?.model, messages: normalizeOpenRouterMessages(messages), tools: openRouterTools(tools), stream: true }); const response = await fetch(request.url, { method: "POST", headers: request.headers, body: JSON.stringify(request.body), signal: controller.signal }); if (!response.ok) return { error: `OpenRouter error: ${response.status} ${response.statusText}`, code: "OPENROUTER_CHAT_FAILED" }; const captured = await captureOpenRouterStream(response.body, { onEvent: (streamEvent) => hooks.onStreamEvent?.(streamEvent), onThinking: (token, streamEvent) => { hooks.onThinking?.(token, streamEvent); send("ollama:thinking", token); }, onContent: (token, streamEvent) => { fullText += token; hooks.onToken?.(token, streamEvent); send("ollama:token", token); }, onToolCalls: (calls, streamEvent) => { toolCalls = calls; hooks.onToolCalls?.(calls, streamEvent); send("ollama:toolcall", calls); } }); fullText = captured.fullText; toolCalls = captured.toolCalls; const payload = { fullText, toolCalls, thinking: captured.thinking, usage: captured.usage, provider: "openrouter" }; send("ollama:done", payload); return { ok: true, ...payload }; } catch (error) { if (controller.signal.aborted) { const payload = { fullText, toolCalls, aborted: true }; send("ollama:done", payload); return { ok: false, ...payload }; } return { error: error.message, code: error.code || "OPENROUTER_CHAT_FAILED", fullText, toolCalls }; } finally { if (llmControllers.get(senderId) === controller) llmControllers.delete(senderId); }
}
...
async function runOpenRouterJson({ model, messages, temperature = 0 }) { const request = buildChatRequest({ baseUrl: getOpenRouterBaseUrl(), apiKey: getOpenRouterApiKey(), model: model || llmPreferences().openrouter?.model, messages: normalizeOpenRouterMessages(messages), stream: false, temperature, responseFormat: { type: "json_object" } }); const response = await fetch(request.url, { method: "POST", headers: request.headers, body: JSON.stringify(request.body) }); if (!response.ok) throw new Error(`OpenRouter error: ${response.status} ${response.statusText}`); const body = await response.json(); return String(body?.choices?.[0]?.message?.content || ""); }
async function summarizeOpenRouterContext(payload) { try { const output = await runOpenRouterJson({ model: payload.model, messages: [{ role: "system", content: ContextMemory.SUMMARY_SYSTEM_PROMPT }, { role: "user", content: ContextMemory.buildMemoryTranscript(payload.previousSummary || "", payload.messages || [], { contextTokens: payload.contextBudget || 4096 }) }], temperature: 0 }); const summary = ContextMemory.normalizeSummary(output, ContextMemory.summaryCharLimit(payload.contextBudget || 4096)); return { ok: summary.length >= 40, summary, source: "model", summarizedMessages: (payload.messages || []).length }; } catch (error) { return { ok: false, error: error.message }; } }
...
ipcMain.handle("ollama:runtime", async (_event, { model } = {}) => {
  if (getActiveProvider() === "openrouter") return { ok: true, loaded: false, model, contextLength: null, provider: "openrouter" };
...
ipcMain.handle("ollama:chat", async (event, { messages, model, numCtx, thinking, tools }) => {
  if (getActiveProvider() === "openrouter") return runOpenRouterChat(event, { messages, model, thinking, tools });
