...
const AUTHENTICATION_REFERENCE = /^[a-zA-Z0-9][a-zA-Z0-9._-]{0,119}$/;
...
function defaultProjectProfile(root, path, now = () => new Date()) {
  ...
  return {
    ...
    engagement: {
      ...
      successCriteria: [],
      deliverables: [],
      executionModel: "operator_choice",
      authenticationSelection: { kind: "none", id: "" },
    },
    authorization: {
      confirmed: false,
      authorizedBy: "",
...
function normalizeProjectProfile(input, root, path, now = () => new Date(), previous = null) {
  ...
  const authenticationSelection = engagement.authenticationSelection && typeof engagement.authenticationSelection === "object" && !Array.isArray(engagement.authenticationSelection)
    ...
    : {};
  const requestedAuthenticationId = text(authenticationSelection.id, "", 120);
  const authenticationId = AUTHENTICATION_REFERENCE.test(requestedAuthenticationId) ? requestedAuthenticationId : "";
  const authenticationKind = ["identity", "credential"].includes(authenticationSelection.kind) && authenticationId
    ? authenticationSelection.kind
    : "none";
  ...
  return {
    ...
    engagement: {
      ...
      executionModel: ["operator_choice", "browser_bound", "standard"].includes(engagement.executionModel)
        ? engagement.executionModel
        : defaults.engagement.executionModel,
      authenticationSelection: {
        kind: authenticationKind,
        id: authenticationKind === "none" ? "" : authenticationId,
      },
    },
    authorization: {
    ...
    scope: {
      ...
      authenticationDependencies: list(scope.authenticationDependencies),
    ...
    dataHandling: {
      ...
      redactSecrets: boolean(dataHandling.redactSecrets, false),
