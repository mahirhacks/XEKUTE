...
const { PHASES, COMPLETION_GATES, TERMINAL_STATUSES } = require("../policies/agentic-loop");
const { CLAIM_STATES } = require("../policies/bugbounty");
...
const UNSUPPORTED_VULN_CLAIM_RE = /\b(?:confirmed|verified|proven)\s+(?:critical\s+|high\s+|medium\s+|low\s+)?(?:vulnerability|finding)|\bis vulnerable\b/i;
const OVERBROAD_SECURE_RE = /\b(?:is|appears|seems)\s+(?:fully\s+)?secure\b/gi;

function hasUnsupportedVulnerabilityClaim(text) {
  return UNSUPPORTED_VULN_CLAIM_RE.test(String(text || ""));
}

function validateFinalClaims(text, { executedTools = false, evidenceIds = [], verification = null, actionResults = [] } = {}) {
  let value = String(text || "").trim();
  const warnings = [];
  if (OVERBROAD_SECURE_RE.test(value)) {
    warnings.push("Overbroad security language was flagged; the response should describe tested conditions instead of claiming the target is secure.");
    value = value.replace(OVERBROAD_SECURE_RE, "had no issue observed under the documented tested conditions");
  }
  if (hasUnsupportedVulnerabilityClaim(value) && !evidenceIds.length) {
    warnings.push("A vulnerability claim lacked admissible evidence; treat it as an inconclusive hypothesis until evidence is linked.");
  }
...
function advancePhase(state, targetPhase, { reason = "Agent turn progression", approvedBy = "runtime" } = {}) {
  const target = String(targetPhase || "").trim();
  if (!target || state?.phase === target) return { ok: true, state };
  const currentIdx = phaseIndex(state.phase);
  const targetIdx = phaseIndex(target);
  if (currentIdx < 0 || targetIdx < 0) {
    return { ok: false, code: "INVALID_PHASE", error: `Unknown phase: ${target}` };
  }
  return transition(state, target, { reason, approvedBy });
}
...
module.exports = {
  ...
  completionIssues,
  validateFinalClaims,
  hasUnsupportedVulnerabilityClaim,
};
