...
(function exposePromptCompiler(globalScope, factory) {
  ...
  // Application consumes generated prompt data through a prompt-source seam.
  // Node uses the application PromptSourcePort adapter; browser uses the
  // preloaded global. Application modules never import content/build directly.
  const promptSource = isNode
    ? require("./prompt-source")
    : { getSystemPrompt: () => globalScope?.XekuteSystemPrompt };
  const source = promptSource.getSystemPrompt();
  const operatingModes = isNode ? require("../policies/operating-mode-rules") : globalScope?.XekuteOperatingModes;
