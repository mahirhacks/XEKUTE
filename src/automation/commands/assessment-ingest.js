...
function loadScopePolicy(workspace) {
  ...
  const settings = readJsonFile(path.join(workspace, "settings.config"), {});
  const unrestricted = ["unrestricted", "full"].includes(String(settings?.authority?.superMode || "").toLowerCase());
  return {
    unrestricted,
...
function validateRecordScope(scopePolicy, resource, record) {
  if (scopePolicy.unrestricted) return { ok: true };
