#!/usr/bin/env python3
"""Parse and run XEKUTE slash commands.
...
def parse_command(raw, overrides=None):
    ...
    if config.get("enabled") is False:
        return {"ok": False, "error": f"Slash command is disabled in XEKUTE Settings: {name}", "code": "COMMAND_DISABLED"}
...
def merge_json(path, command, target, results, assets, endpoints, pages):
    ...
    run = {"id": run_id, "startedAt": stamp, "completedAt": stamp, "operator": "XEKUTE slash command", "tool": command, "status": "completed" if any(item.get("exitCode") == 0 for item in results) else "partial", "targetIds": [target], "outputFiles": [item.get("outputFile") for item in results if item.get("outputFile")], "notes": "Generated by the local Python slash-command runner."}
