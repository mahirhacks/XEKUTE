...
// Resolver seam: DNS/address lookup is injectable so domain scope rules stay
// unit-testable without a concrete network implementation. The default uses
// node:dns and matches the pre-restructure behavior exactly.
function defaultResolver() {
  return {
    async lookup(hostname, options) {
      return dns.lookup(hostname, { all: true, verbatim: true, ...options });
    },
  };
}
...
async function resolveTargetAddresses(rawTarget, { lookup } = {}) {
  ...
  const resolver = typeof lookup === "function" ? { lookup } : defaultResolver();
  let addresses;
  try {
    const records = target.isIp
      ? [{ address: target.hostname }]
      : await resolver.lookup(target.hostname, { all: true, verbatim: true });
    addresses = (Array.isArray(records) ? records : []).map((record) => String(record.address)).filter((address) => net.isIP(address));
  } catch (error) {
...
module.exports = { defaultResolver, normalizeHostname, canonicalTarget, cidrContains, isPrivateOrReservedIp, resolveTargetAddresses, compareResolution, evaluateTarget, extractCommandTargets, testingWindowAllows };
