...
function createAgentTerminalRunner({
  ...
  function spawnShellSession(webContents, workspace, { sendAgentEvent, toolName = "run_command", command = "" } = {}) {
  ...
  function runExecutable(webContents, workspace, executable, args = [], {
    ...
    return new Promise((resolve) => {
      ...
      try {
        const id = nextAgentTerminalId();
        const term = pty.spawn(executable, args, {
          name: "xterm-color",
          cwd: resolved.root,
          env: { ...process.env, TERM: "xterm-256color" },
          cols: 120,
  ...
  function startProcess(webContents, workspace, command, { sendAgentEvent, ownerId = "agent" } = {}) {
    const text = String(command || "").trim();
