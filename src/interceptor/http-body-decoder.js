...
const DEFAULT_MAX_DECODED_BYTES = 8 * 1024 * 1024;
...
async function tryDecompress(fn, buf, maxOutputLength = DEFAULT_MAX_DECODED_BYTES) {
  try {
    return await fn(buf, { maxOutputLength });
  } catch {
    return null;
  }
}

async function decompressBuffer(buffer, encoding, maxOutputLength = DEFAULT_MAX_DECODED_BYTES) {
  const outputLimit = Math.max(1024, Math.min(Number(maxOutputLength) || DEFAULT_MAX_DECODED_BYTES, 64 * 1024 * 1024));
  const enc = String(encoding || "").toLowerCase().trim();
  if (enc === "gzip" || enc === "x-gzip") {
    const result = await tryDecompress(gunzip, buffer, outputLimit);
    if (result) return result;
  } else if (enc === "deflate") {
    const result = await tryDecompress(inflate, buffer, outputLimit) ?? await tryDecompress(inflateRaw, buffer, outputLimit);
    if (result) return result;
  } else if (enc === "br") {
    const result = await tryDecompress(brotliDecompress, buffer, outputLimit);
    if (result) return result;
  }
...
async function decodeHttpBodyBuffer(buffer, contentEncoding, maxOutputLength = DEFAULT_MAX_DECODED_BYTES) {
  if (!buffer || buffer.length === 0) return Buffer.alloc(0);
  return decompressBuffer(buffer, contentEncoding, maxOutputLength);
}
