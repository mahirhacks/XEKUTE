...
function usageFromResponse(usage) {
  ...
  return { promptTokens: Number.isFinite(promptTokens) ? promptTokens : null, completionTokens: Number.isFinite(completionTokens) ? completionTokens : null, totalTokens: Number(usage.total_tokens) || (promptTokens || 0) + (completionTokens || 0), source: "openrouter" };
}
...
async function captureOpenRouterStream(readable, callbacks = {}, options = {}) {
  const idleTimeoutMs = Number.isFinite(Number(options.idleTimeoutMs))
    ...
    : 120000;
  const totalTimeoutMs = Math.max(idleTimeoutMs, Number(options.totalTimeoutMs) || 300000);
  ...
  let buffer = "", sequence = 0, fullText = "", thinking = "", toolCalls = [], usage = null, done = false;
  ...
  const result = { fullText, thinking, toolCalls, done, sequence, usage };
...
module.exports = { captureOpenRouterStream, mergeToolCalls, normalizeOpenRouterMessages, openRouterHeaders, openRouterTools, usageFromResponse };
