...
const { MODE_KEY_ALIASES, MODES } = require("../../../agent/modes/mode-registry.js");
...
function internalSkillSelection(raw, knownIds = INTERNAL_SKILL_IDS) {
...
function createSpecialSkillRegistry({ root = pathDefault.resolve(__dirname, "../libraries"), fs, path = pathDefault } = {}) {
  let snapshot = null;
  function load() {
    if (!snapshot) {
  ...
  function select(raw, options = {}) {
    const knownIds = new Set([...INTERNAL_SKILL_IDS, ...load().byId.keys()]);
    const selection = internalSkillSelection(raw, knownIds);
