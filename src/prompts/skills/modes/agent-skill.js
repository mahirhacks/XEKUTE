...
const TESTING_AGENT = [
  ...
  "The exposed catalog contains the canonical tools: exec_command, read_file, search_workspace, apply_patch, inspect_environment, manage_plan, manage_state, ingest_traffic, manage_identity, replay_request, run_test_case, browser_action, compare_responses, verify_finding, store_finding, attack_graph, delegate_agent, query_assessment, expand_evidence, query_knowledge, and web_research.",
  ...
  "For commands expected to run longer than a normal interactive action, use exec_command with operation=start. Record the returned processId, continue other useful work, then use operation=status with that process_id. When waiting is the useful next action, set wait_ms to a sensible observation window instead of polling rapidly; pass the returned stdout_offset and stderr_offset cursors on later checks to receive only new output. An observation timeout does not stop the process.",
