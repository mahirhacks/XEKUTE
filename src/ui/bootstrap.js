...
const generalSettingsNotice = $("general-settings-notice");
const generalLayoutButtons = [...document.querySelectorAll("[data-general-layout]")];
const generalConversationDensity = $("general-conversation-density");
...
const guidanceNewKind = $("guidance-new-kind");
...
const ollamaHostSave = $("ollama-host-save");
...
const APP_SETTINGS_SECTION_META = Object.freeze({
  ...
  commands: { title: "Commands", subtitle: "Configure reusable workflows and execution behavior" },
  ...
  llm: { title: "Models", subtitle: "Choose and configure the active model provider" },
...
function normalizePersistedChatSession(value) {
  ...
  const history = Array.isArray(sourceMessages)
    ? sourceMessages
      ...
      .map(({ thinking: _privateReasoning, ...message }) => message)
    : [];
...
const AUTHORITY_GROUPS = [
  ["Workspace", "AI access to files, local processes, and terminal capabilities.", [
    ["workspaceRead", "Read workspace files", "Allow the AI agent to inspect project and assessment files."],
    ["workspaceWrite", "Create and edit files", "Allow the AI agent to write, patch, and generate workspace content."],
    ["workspaceDelete", "Delete files and folders", "Allow the AI agent to remove user-created workspace content."],
    ["commandExecution", "Run local commands", "Allow the AI agent to execute foreground commands through typed adapters."],
    ["backgroundProcesses", "Start background processes", "Allow the AI agent to start long-running tools and managed processes."],
    ["terminalAccess", "Use terminal capabilities", "Allow the AI agent to use terminal-backed workflows."],
    ["customScripts", "Run custom scripts", "Allow the AI agent to run scripts from the custom scripts folder."],
  ]],
  ["Network and Traffic", "AI access to outbound requests, interception, capture, and research.", [
    ["webResearch", "Web research", "Allow the AI agent to search and retrieve readable public pages."],
    ["outboundHttp", "Send HTTP requests", "Allow the AI agent to use HTTP adapters and endpoint probes."],
    ["proxyInterception", "Control proxy interception", "Allow the AI agent to control interception and request forwarding."],
    ["trafficCapture", "Capture request/response traffic", "Allow AI workflows to persist Traffic/Raw logs and response evidence."],
    ["sensitiveDataAccess", "Read unredacted sensitive data", "Allow the AI agent to access credentials, tokens, cookies, and unredacted bodies."],
  ]],
  ["Assessment", "AI access to evidence, graph, reporting, and passive discovery.", [
    ["mapBuild", "Build and query the Map", "Allow the AI agent to generate and query application graph evidence."],
    ["evidenceManagement", "Manage evidence and findings", "Allow the AI agent to manage evidence, hypotheses, findings, and reports."],
    ["passiveRecon", "Run passive reconnaissance", "Allow the AI agent to perform public-source discovery without active probing."],
  ]],
  ["Sensitive Testing", "AI capabilities that may send substantial traffic or validate vulnerabilities.", [
    ["activeRecon", "Run active reconnaissance", "Allow the AI agent to probe approved in-scope assets."],
    ["automatedScanning", "Run automated scanners", "Allow the AI agent to use tools such as Nmap, ffuf, Nuclei, Nikto, and Katana."],
    ["exploitValidation", "Validate exploit hypotheses", "Allow the AI agent to perform explicitly authorized exploit checks in Test mode."],
  ]],
];
...
const GUIDANCE_KIND_LABELS = Object.freeze({ skills: "Skill", rules: "Rule", instructions: "Instruction" });
const GUIDANCE_KIND_ICONS = Object.freeze({ skills: "codicon-symbol-keyword", rules: "codicon-law", instructions: "codicon-note" });

function guidanceKindLabel(kind) {
  return GUIDANCE_KIND_LABELS[String(kind || "").toLowerCase()] || "Instruction";
}
...
function guidanceEntryKind(entry) {
  ...
  return GUIDANCE_KIND_LABELS[parts[1]] ? parts[1] : "instructions";
}
...
function setGuidanceEmptyState(message = "Create or import a skill, rule, or instruction file for this project.") {
  if (guidanceSettingsEmpty) {
    guidanceSettingsEmpty.hidden = false;
    const messageEl = guidanceSettingsEmpty.querySelector("p");
...
function renderGuidanceList() {
  ...
  [guidanceNewKind, guidanceNew, guidanceImport, guidanceEmptyNew].forEach((element) => { if (element) element.disabled = !hasProject; });
  if (!hasProject) {
    guidanceSettingsList.innerHTML = '<div class="guidance-list-empty"><span class="codicon codicon-folder-library"></span><strong>Open a project first</strong><small>Project guidance is stored with the active workspace.</small></div>';
    setGuidanceEmptyState("Open a project to add custom skills, rules, and instructions.");
    return;
  }
  if (guidanceSettingsList) {
    guidanceSettingsList.innerHTML = customGuidanceEntries.length
      ? customGuidanceEntries.map((entry) => {
        const kind = guidanceEntryKind(entry);
        const selected = entry.relativePath === selectedGuidancePath;
        return `<button type="button" class="guidance-list-item${selected ? " active" : ""}" data-guidance-path="${escapeHtml(entry.relativePath)}" aria-pressed="${selected}"><span class="codicon ${GUIDANCE_KIND_ICONS[kind]}"></span><span><strong>${escapeHtml(entry.name || entry.relativePath.split("/").pop())}</strong><small>${escapeHtml(guidanceKindLabel(kind))} - ${escapeHtml(entry.relativePath)}</small></span></button>`;
      }).join("")
      : '<div class="guidance-list-empty"><span class="codicon codicon-symbol-keyword"></span><strong>No custom guidance yet</strong><small>Use New file or Import to add one.</small></div>';
    guidanceSettingsList.querySelectorAll("[data-guidance-path]").forEach((button) => button.addEventListener("click", () => selectGuidanceEntry(button.dataset.guidancePath)));
  }
  if (selectedGuidancePath && customGuidanceEntries.some((entry) => entry.relativePath === selectedGuidancePath)) renderGuidanceDetail();
  else setGuidanceEmptyState();
}

function renderGuidanceDetail() {
  if (!guidanceSettingsDetail) return;
  const existing = customGuidanceEntries.find((entry) => entry.relativePath === selectedGuidancePath);
  const draft = guidanceDraft || (existing ? { ...existing, content: selectedGuidanceContent, isNew: false } : null);
  if (!draft) {
    setGuidanceEmptyState();
    return;
  }
  const kind = guidanceEntryKind(draft);
  const isNew = draft.isNew === true;
  const fileName = isNew ? String(draft.fileName || "") : String(draft.name || draft.relativePath?.split("/").pop() || "");
  const pathValue = isNew ? `custom/${kind}/` : String(draft.relativePath || "");
  guidanceSettingsEmpty.hidden = true;
  guidanceSettingsDetail.hidden = false;
  guidanceSettingsDetail.innerHTML = `<header class="guidance-detail-header"><div><span class="codicon ${GUIDANCE_KIND_ICONS[kind]}"></span><div><h2>${escapeHtml(isNew ? "New guidance file" : fileName)}</h2><p>${escapeHtml(isNew ? "Add project-specific context without changing XEKUTE's protected system instructions." : draft.relativePath)}</p></div></div><span class="guidance-kind-badge">${escapeHtml(guidanceKindLabel(kind))}</span></header><div class="guidance-detail-form"><div class="guidance-detail-grid"><label>Type<select id="guidance-file-kind" ${isNew ? "" : "disabled"}><option value="skills" ${kind === "skills" ? "selected" : ""}>Skill</option><option value="rules" ${kind === "rules" ? "selected" : ""}>Rule</option><option value="instructions" ${kind === "instructions" ? "selected" : ""}>Instruction</option></select></label><label>File name<input id="guidance-file-name" value="${escapeHtml(fileName)}" placeholder="example.md" ${isNew ? "" : "readonly"} spellcheck="false"></label></div><label>Location<input class="guidance-file-location" value="${escapeHtml(pathValue)}" readonly spellcheck="false"></label><label>Content<textarea id="guidance-file-content" spellcheck="false" placeholder="Write the workflow, rule, or reference material the agent should use.">${escapeHtml(draft.content || "")}</textarea></label><aside class="guidance-protection-note"><span class="codicon codicon-shield"></span><p>This file is additive context only. It cannot grant permissions, bypass authorization, change scope, or override runtime guardrails.</p></aside></div><footer class="guidance-detail-actions"><button type="button" id="guidance-cancel-file" class="secondary-button">Cancel</button><button type="button" id="guidance-delete-file" class="secondary-button" ${isNew ? "hidden" : ""}><span class="codicon codicon-trash"></span>Delete</button><button type="button" id="guidance-save-file" class="primary-button"><span class="codicon codicon-save"></span>${isNew ? "Create file" : "Save file"}</button></footer>`;
  const kindInput = guidanceSettingsDetail.querySelector("#guidance-file-kind");
  const nameInput = guidanceSettingsDetail.querySelector("#guidance-file-name");
  const contentInput = guidanceSettingsDetail.querySelector("#guidance-file-content");
  const markDirty = () => {
    if (guidanceDraft) guidanceDraft.dirty = true;
    if (contentInput) contentInput.dataset.dirty = "true";
    if (commandSettingsStatus) commandSettingsStatus.textContent = "Unsaved guidance changes";
  };
  kindInput?.addEventListener("change", () => { if (guidanceDraft) guidanceDraft.kind = kindInput.value; markDirty(); renderGuidanceDetail(); });
  nameInput?.addEventListener("input", () => { if (guidanceDraft) guidanceDraft.fileName = nameInput.value; markDirty(); });
  contentInput?.addEventListener("input", () => { selectedGuidanceContent = contentInput.value; if (guidanceDraft) guidanceDraft.content = contentInput.value; markDirty(); });
  guidanceSettingsDetail.querySelector("#guidance-cancel-file")?.addEventListener("click", async () => {
    const pathToReload = selectedGuidancePath;
    const wasNew = guidanceDraft?.isNew === true;
    guidanceDraft = null;
    if (!wasNew && pathToReload) {
      const result = await window.api.guidanceRead?.({ workspace: rootPath, relativePath: pathToReload });
      selectedGuidanceContent = result?.content || "";
      renderGuidanceList();
      return;
    }
...
async function selectGuidanceEntry(relativePath) {
  if (guidanceDetailIsDirty() && !window.confirm("Discard unsaved guidance changes?")) return;
  const result = await window.api.guidanceRead?.({ workspace: rootPath, relativePath });
  if (result?.error) {
    if (commandSettingsStatus) commandSettingsStatus.textContent = result.error;
    return;
  }
  selectedGuidancePath = relativePath;
...
function beginGuidanceCreate() {
  if (!rootPath) return;
  guidanceDraft = { isNew: true, kind: guidanceNewKind?.value || "skills", fileName: "", content: "", dirty: false };
  selectedGuidancePath = "";
  selectedGuidanceContent = "";
  renderGuidanceDetail();
  guidanceSettingsDetail?.querySelector("#guidance-file-name")?.focus();
}

async function saveGuidanceFile() {
  if (!guidanceSettingsDetail || !rootPath) return;
  const existing = customGuidanceEntries.find((entry) => entry.relativePath === selectedGuidancePath);
  const isNew = guidanceDraft?.isNew === true;
  const kind = isNew ? guidanceDraft.kind : guidanceEntryKind(existing);
  const fileName = isNew ? guidanceSettingsDetail.querySelector("#guidance-file-name")?.value.trim() : existing?.name;
  const content = guidanceSettingsDetail.querySelector("#guidance-file-content")?.value || "";
  if (!fileName || !/^[^\\/:*?"<>|]+\.(?:md|markdown|txt|yaml|yml|json)$/i.test(fileName)) {
    if (commandSettingsStatus) commandSettingsStatus.textContent = "Choose a valid guidance filename ending in .md, .txt, .yaml, or .json";
    guidanceSettingsDetail.querySelector("#guidance-file-name")?.focus();
    return;
  }
  const relativePath = isNew ? `custom/${kind}/${fileName}` : selectedGuidancePath;
  const result = await window.api.guidanceSave?.({ workspace: rootPath, relativePath, content });
  if (result?.error) {
    if (commandSettingsStatus) commandSettingsStatus.textContent = result.error;
    return;
  }
...
async function deleteGuidanceFile() {
  if (!rootPath || !selectedGuidancePath || !window.confirm(`Delete ${selectedGuidancePath}?`)) return;
  const result = await window.api.guidanceDelete?.({ workspace: rootPath, relativePath: selectedGuidancePath });
  if (result?.error) {
    if (commandSettingsStatus) commandSettingsStatus.textContent = result.error;
    return;
  }
  ...
  selectedGuidanceContent = "";
  guidanceDraft = null;
  if (commandSettingsStatus) commandSettingsStatus.textContent = "Guidance file deleted";
  await loadGuidanceSettings();
}

async function importGuidanceFiles() {
  if (!rootPath) return;
  const result = await window.api.guidanceImport?.({ workspace: rootPath, kind: guidanceNewKind?.value || "skills" });
  if (result?.canceled) return;
  if (result?.error) {
    if (commandSettingsStatus) commandSettingsStatus.textContent = result.error;
    return;
  }
  const imported = Array.isArray(result.paths) ? result.paths : [];
  selectedGuidancePath = imported[imported.length - 1] || "";
  if (commandSettingsStatus) commandSettingsStatus.textContent = imported.length ? `Imported ${imported.length} guidance file${imported.length === 1 ? "" : "s"}` : "No supported guidance files were imported";
  await loadGuidanceSettings({ preserveSelection: true });
  if (selectedGuidancePath) await selectGuidanceEntry(selectedGuidancePath);
}

function renderGuidanceSettings() {
  renderGuidanceList();
  if (guidanceDraft) renderGuidanceDetail();
}

async function loadGuidanceContext() {
  if (!rootPath || !window.api.guidanceContext) {
    guidanceContext = "";
    guidanceContextRequestPath = "";
    return guidanceContext;
  }
  const requestedWorkspace = rootPath;
  ...
  guidanceContextPromise = window.api.guidanceContext({ workspace: requestedWorkspace })
    .then((result) => {
      if (rootPath === requestedWorkspace) guidanceContext = result?.context || "";
      return guidanceContext;
    })
...
async function loadGuidanceSettings({ preserveSelection = false } = {}) {
  ...
  guidanceDraft = preserveSelection ? guidanceDraft : null;
  if (!rootPath || !window.api.guidanceEntries) {
  ...
  const result = await window.api.guidanceEntries({ workspace: rootPath });
  customGuidanceEntries = Array.isArray(result?.entries) ? result.entries : [];
  if (preserveSelection && previousSelection && customGuidanceEntries.some((entry) => entry.relativePath === previousSelection)) selectedGuidancePath = previousSelection;
  else if (!customGuidanceEntries.some((entry) => entry.relativePath === selectedGuidancePath)) selectedGuidancePath = "";
  renderGuidanceSettings();
...
const GENERAL_SETTINGS_DEFAULTS = Object.freeze({
  layout: "editor",
...
function readGeneralSettings() {
  ...
  return {
    layout: stored.layout === "agent" ? "agent" : "editor",
    conversationDensity: stored.conversationDensity === "compact" ? "compact" : "detailed",
    showStatusBar: stored.showStatusBar !== false,
  };
}

function showGeneralSettingsNotice(message = "") {
  if (!generalSettingsNotice) return;
  generalSettingsNotice.textContent = message;
  generalSettingsNotice.hidden = !message;
}

function renderGeneralSettings() {
  const settings = readGeneralSettings();
  generalLayoutButtons.forEach((button) => {
    const active = button.dataset.generalLayout === settings.layout;
    button.classList.toggle("active", active);
    button.setAttribute("aria-pressed", String(active));
  });
  if (generalConversationDensity) generalConversationDensity.value = settings.conversationDensity;
  ...
  document.documentElement.dataset.xekuteLayoutPreference = settings.layout;
  document.documentElement.dataset.xekuteConversationDensity = settings.conversationDensity;
}
...
function setAppSettingsSection(section) {
  ...
  if (appSettingsSection === "general") {
    showGeneralSettingsNotice("");
    renderGeneralSettings();
  }
  if (appSettingsSection === "project") loadProjectProfile();
  if (appSettingsSection === "authority") renderAuthoritySettings();
  if (appSettingsSection === "prompts") renderGuidanceSettings();
  if (appSettingsSection === "llm") { loadOllamaSettings(); loadLlmSettings(); }
...
function syncLlmProviderUi(provider = "ollama") {
  ...
  document.querySelectorAll(".llm-provider-option").forEach((option) => {
    const input = option.querySelector('input[name="llm-provider"]');
...
async function loadLlmSettings() {
  ...
  if (openRouterBaseUrl) openRouterBaseUrl.value = result.openrouter?.baseUrl || "https://openrouter.ai/api/v1";
...
async function saveLlmSettings() {
  ...
  const result = await window.api.setLlmSettings?.({
    provider,
    baseUrl: openRouterBaseUrl?.value,
...
async function saveOllamaSettings() {
  ...
  if (ollamaConnectionStatus) ollamaConnectionStatus.textContent = "Saving…";
  ...
  renderOllamaSettings(result);
  if (commandSettingsStatus) commandSettingsStatus.textContent = "Ollama endpoint saved";
  await loadModels({ showLoading: true });
}
...
function renderAuthoritySettings() {
  ...
  const superOptions = [
    ["full", "Full Authority", "Forces every listed AI permission ON and bypasses agent approval prompts.", "Overrides all AI permission switches and approval behavior.", "danger"],
    ["ask", "Ask for Approval", "Uses the detailed AI permission switches and asks before sensitive or mutating agent actions.", "Recommended for human-in-the-loop work.", ""],
    ["approve", "Approve for me", "Uses the detailed AI permission switches and automatically approves allowed agent actions.", "Does not enable disabled AI permissions.", ""],
  ];
  const modeSummary = mode === "full"
    ? '<strong>Effective override:</strong> every AI permission below is ON and agent approval prompts are bypassed. Safe/Test mode and engagement scope remain operational boundaries.'
    : mode === "approve"
      ? '<strong>Effective override:</strong> enabled AI permissions are automatically approved. Disabled permissions still block agent and automated actions.'
      : '<strong>Effective override:</strong> detailed AI permissions remain authoritative and sensitive agent actions require operator approval.';
  ...
  authoritySettingsContent.innerHTML = `<div class="authority-intro"><h2>AI Authority</h2><p>Control what the AI agent and automated workflows may read, change, execute, send, capture, and validate. These switches never restrict your own direct actions in the UI, including creating, editing, saving, renaming, moving, or deleting project content. Engagement scope and authorization remain separately documented under Project.</p></div><div class="authority-super-grid">${superOptions.map(([key, label, description, override, tone]) => `<label class="authority-super-option ${tone}${mode === key ? " selected" : ""}"><input type="radio" name="authority-super" value="${key}" ${mode === key ? "checked" : ""}><strong>${label}</strong><small>${description}</small><em>${override}</em></label>`).join("")}</div><div class="authority-override-summary ${mode === "full" ? "danger" : mode === "ask" ? "warning" : ""}">${modeSummary}</div><div class="authority-groups">${groups}</div>`;
  authoritySettingsContent.querySelectorAll('input[name="authority-super"]').forEach((input) => input.addEventListener("change", () => {
    if (input.value === "full" && !window.confirm("Full Authority enables every listed AI capability and bypasses agent approval prompts. Engagement scope still applies. Continue?")) {
...
function modeTools(mode = chatMode) {
  const profile = CHAT_PROFILE_DEFS[canonicalChatMode(mode, mode.includes(":") ? mode.split(":")[0] : chatFamily)] || {};
  return ToolMap.toolsForProfile(profile.family || chatFamily, profile.key || "ask");
}
...
function getContextBreakdown(requestText = chatInput.value.trim()) {
  ...
  const latestUserText = [...chatHistory].reverse().find((message) => message?.role === "user")?.content || "";
  const routedText = requestText || latestUserText;
  const route = contextPreviewRoute(routedText);
  const routedTools = ToolMap.compactTools(ToolMap.toolsForRoute(modeTools(), route));
  const compiledPrompt = globalThis.XekutePromptCompiler?.compile({
    family: chatFamily,
    mode: chatMode,
    depth: route.promptDepth,
  }) || ToolParser.SYSTEM_PROMPT || "";
  const toolMenu = globalThis.XekuteInitialPrompts?.toolMenu(routedTools, ToolMap.TOOL_META) || "";
  ...
  const summaryMessage = route.includeMemory
    ? buildSummaryContextMessage(activeChatSession()?.contextSummary)
    : "";
...
function normalizeContextUsageSnapshot(value) {
  ...
  return {
    version: 1,
    source: value.source === "ollama" ? "ollama" : "estimate",
    promptTokens,
    ...
    contextWindowSource: ["manual", "runtime", "fallback"].includes(value.contextWindowSource) ? value.contextWindowSource : "fallback",
...
function storeLastContextUsage(value) {
  ...
  if (settings.context !== AUTO_CONTEXT) {
    usage.contextWindow = contextToTokens(settings.context);
    usage.contextWindowSource = "manual";
  }
...
async function refreshStoredContextCapacity() {
  ...
  const settings = selectedModel ? getModelSettings(selectedModel) : { context: AUTO_CONTEXT };
  if (!usage || settings.context !== AUTO_CONTEXT || !selectedModel || !window.api?.runtimeModel) return;
  try {
    const runtime = await window.api.runtimeModel({ model: selectedModel });
    if (!runtime?.ok || !runtime.loaded || !Number.isFinite(Number(runtime.contextLength)) || Number(runtime.contextLength) <= 0) return;
    if (session !== activeChatSession()) return;
    usage.contextWindow = Number(runtime.contextLength);
    usage.contextWindowSource = "runtime";
    syncActiveChatSession();
    updateContextUsage();
  } catch {
    /* Keep the configured fallback capacity. */
  }
}
...
function getContextUsage(usedOverride = null) {
  ...
  const stored = !draft && !streaming ? normalizeContextUsageSnapshot(activeChatSession()?.lastContextUsage) : null;
  ...
  const total = stored?.contextWindow || contextToTokens(settings.context);
  ...
  return {
    ...
    breakdown,
    source: stored?.source === "ollama" ? "actual" : "estimate",
    capacityApproximate: settings.context === AUTO_CONTEXT && stored?.contextWindowSource !== "runtime",
  };
}
...
function renderContextUsage({ total, used, free, pct, source, breakdown = getContextBreakdown(), capacityApproximate = true }) {
  if (!contextRingFill) return;
  const filled = pct * CONTEXT_RING_C;
  ...
  if (contextUsageUsed) contextUsageUsed.textContent = `${Math.round(pct * 100)}% used`;
  ...
  if (contextUsageSource) {
    contextUsageSource.textContent = actual ? "Measured" : "Estimate";
  }
  ...
  if (contextUsageMeasureNote) {
    contextUsageMeasureNote.textContent = actual
      ? `Total combines Ollama's prompt_eval_count and eval_count from the last model turn. Input breakdown rows are proportional estimates. Ollama receives this context per request; there is no separate hidden chat memory.${capacityApproximate ? " Context capacity is still an Auto-mode estimate." : ""}`
      : "Preview of the next routed payload. The final total is replaced by Ollama's measured prompt and output counts after the request.";
  }
  const session = activeChatSession();
...
async function deleteSelectedCustomEntries() {
  ...
  try {
    ...
    alert(`Delete failed: ${error?.message || "Unexpected workspace error"}`);
  } finally {
...
projectSettingsNavButtons.forEach((button) => button.addEventListener("click", () => {
  const target = $(button.dataset.projectSettingsTarget);
  if (!target || !projectSettingsForm) return;
  projectSettingsNavButtons.forEach((item) => item.classList.toggle("active", item === button));
  projectSettingsForm.scrollTo({ top: Math.max(0, target.offsetTop - 12), behavior: "smooth" });
}));
...
ollamaHostSave?.addEventListener("click", saveOllamaSettings);
...
document.querySelectorAll('input[name="llm-provider"]').forEach((input) => {
  input.addEventListener("change", () => {
    if (!input.checked) return;
    syncLlmProviderUi(input.value);
    if (llmSettingsStatus) {
      llmSettingsStatus.textContent = input.value === "openrouter"
...
generalLayoutButtons.forEach((button) => button.addEventListener("click", () => {
  const layout = button.dataset.generalLayout === "agent" ? "agent" : "editor";
  updateGeneralSettings({ layout });
  showGeneralSettingsNotice("Default " + (layout === "agent" ? "Agent" : "Editor") + " layout selected.");
}));
generalConversationDensity?.addEventListener("change", () => {
  updateGeneralSettings({ conversationDensity: generalConversationDensity.value === "compact" ? "compact" : "detailed" });
  showGeneralSettingsNotice("Conversation density preference saved.");
});
generalStatusBarToggle?.addEventListener("change", () => {
  updateGeneralSettings({ showStatusBar: generalStatusBarToggle.checked });
  showGeneralSettingsNotice(generalStatusBarToggle.checked ? "Status bar shown." : "Status bar hidden.");
});
document.querySelectorAll("[data-general-settings-action]").forEach((button) => button.addEventListener("click", () => {
  const action = button.dataset.generalSettingsAction;
  const messages = {
    account: "Account settings are not connected in this local build.",
    editor: "Editor preferences will be available in a future settings update.",
    shortcuts: "Keyboard shortcut preferences will be available in a future settings update.",
    import: "VS Code settings import is not available in this local build.",
    "reset-dialogs": "There are no hidden XEKUTE dialogs to restore.",
  };
  showGeneralSettingsNotice(messages[action] || "This setting is not available yet.");
}));
document.querySelectorAll("[data-settings-placeholder]").forEach((button) => button.addEventListener("click", () => {
  if (commandSettingsStatus) commandSettingsStatus.textContent = `${button.textContent.replace(/\s+/g, " ").trim()} is not available yet`;
}));
...
function buildLegacyCommandItems() {
  ...
  return [
    ...
    {
      title: "View: Project",
      detail: "Show the project folder explorer",
      icon: "codicon-folder-library",
      run: () => activateSidebarView("project"),
    },
    {
      title: "Project: Create New Project",
      detail: "Create a blank project folder without scaffolding files",
...
async function deleteSelectedExplorerItem() {
  ...
  if (btnDeleteItem) btnDeleteItem.disabled = true;
  try {
...
function closeTab(filePath, e, { force = false } = {}) {
  e?.stopPropagation();
  if (!openTabs.has(filePath)) return;
...
function renderTabs() {
  ...
  for (const [path, tab] of openTabs) {
    ...
    el.title = relativePathFromRoot(tab.diskPath) || tab.diskPath || tab.name;
...
async function renderEditor() {
  ...
  if (tab.content === null && !tab.error) {
    ...
    try {
      ...
      editorError.hidden = false;
      editorError.textContent = `Editor failed to load: ${error?.message || error}`;
    }
    return;
  }
...
function applyOllamaListResult(result) {
  ...
  const preferred = result.provider === "openrouter"
    ...
    : selectedModel;
  const custom = preferred && !allModels.includes(preferred) ? [preferred] : [];
  allModels = normalizeModelNames([...custom, ...allModels]);
  if (preferred && allModels.includes(preferred)) {
    selectedModel = preferred;
    localStorage.setItem("pointer:model", selectedModel);
  } else if (!selectedModel && allModels.length) {
...
function buildMessagesForApi() {
  ...
  if (guidanceContext) msgs.push({ role: "system", content: guidanceContext });
...
async function applyToolResultToUi(tool, result, turn, contentEl) {
  ...
  let successText = ToolParser.formatToolSuccess(result);
  if (result?.terminalId) {
    successText = `${successText} View output in the AI terminal tab.`;
...
function waitForOllamaRound(assistant, editContext) {
  return new Promise((resolve) => {
    ...
    window.api.onThinking(() => {
      if (assistant.showPrivateReasoning()) {
...
function taskStepIcon(status) {
  if (status === "done") return "codicon-check";
  if (status === "active") return "codicon-loading codicon-modifier-spin";
...
function activityIcon(kind = "info") {
  return {
    meta: "codicon-info",
    info: "codicon-info",
    status: "codicon-pulse",
    thinking: "codicon-loading codicon-modifier-spin",
    tool: "codicon-tools",
    success: "codicon-check",
    warn: "codicon-warning",
    error: "codicon-error",
  }[kind] || "codicon-circle-filled";
}

function taskPhaseStepId(phase = "") {
  const value = String(phase || "").toLowerCase().replace(/[-_]/g, " ");
  if (value.includes("preflight") || value.includes("approval") || value.includes("scope") || value.includes("policy")) return "preflight";
  if (value.includes("inventory") || value.includes("discovery") || value.includes("inspect") || value.includes("context")) return "inspect";
  if (value.includes("hypothesis") || value.includes("test design") || value.includes("plan")) return "plan";
  if (value.includes("execution") || value.includes("action") || value.includes("observation") || value.includes("working")) return "execute";
  if (value.includes("verification") || value.includes("finding") || value.includes("report") || value.includes("complete")) return "verify";
  return "";
}
...
function createAssistantTurn() {
  ...
  const assistant = {
    ...
    ensureLiveState() {
      ...
      block.className = "agent-live-state";
      block.setAttribute("aria-live", "polite");
      block.innerHTML = `
        <span class="agent-live-state-icon codicon codicon-pulse" aria-hidden="true"></span>
        <div class="agent-live-state-copy"><strong>Working</strong><span>Preparing the next step…</span></div>
        <span class="agent-live-state-meta">LIVE</span>
      `;
    ...
    setLiveState({ kind = "working", title = "Working", detail = "", meta = "LIVE" } = {}) {
      ...
      const stateKey = `${kind}|${title}|${detail}|${meta}`;
      if (block.dataset.stateKey === stateKey) return;
      block.dataset.stateKey = stateKey;
      block.dataset.state = kind;
      const icon = block.querySelector(".agent-live-state-icon");
      if (icon) icon.className = `agent-live-state-icon codicon ${agentStateIcon(kind)}`;
      const titleEl = block.querySelector(".agent-live-state-copy strong");
      const detailEl = block.querySelector(".agent-live-state-copy span");
      const metaEl = block.querySelector(".agent-live-state-meta");
      if (titleEl) titleEl.textContent = title;
      if (detailEl) detailEl.textContent = detail || "Working…";
      if (metaEl) metaEl.textContent = meta;
      block.classList.remove("state-pulse");
      void block.offsetWidth;
      block.classList.add("state-pulse");
      scrollMessages();
    },
    requestDecision({ tool = "action", target = "workspace", capability = "unknown", risk = "unknown", reason = "This action needs your approval.", expiresInMs = 60_000 } = {}) {
      ...
      block.setAttribute("aria-label", "Action approval required");
      block.innerHTML = `
        <div class="agent-decision-header">
          <span class="agent-decision-icon codicon codicon-shield" aria-hidden="true"></span>
          <div><strong>Allow this action?</strong><span>The agent paused before using a gated capability.</span></div>
          <span class="agent-decision-live">ACTION PAUSED</span>
        </div>
        <p class="agent-decision-reason">${escapeHtml(reason)}</p>
        <dl class="agent-decision-details">
          <div><dt>Action</dt><dd>${escapeHtml(tool)}</dd></div>
          <div><dt>Target</dt><dd title="${escapeHtml(target)}">${escapeHtml(target)}</dd></div>
          <div><dt>Capability</dt><dd>${escapeHtml(capability)}</dd></div>
          <div><dt>Risk</dt><dd>${escapeHtml(risk)}</dd></div>
        </dl>
    ...
    ensureTaskBrief(brief) {
      ...
      let block = this.taskBriefEl;
      if (!block) {
        this.turn.classList.add("has-agent-run");
        block = document.createElement("section");
        block.className = "agent-task-brief is-active";
        this.turn.insertBefore(block, this.turn.firstChild);
        this.taskBriefEl = block;
      }
      this.taskBrief = brief;
      block.dataset.taskStatus = "active";
      const rows = brief.steps.map((step) => `
        <li class="agent-task-step" data-task-step="${escapeHtml(step.id)}" data-task-status="pending">
          <span class="agent-task-step-icon codicon ${taskStepIcon("pending")}" aria-hidden="true"></span>
          <span class="agent-task-step-copy"><strong>${escapeHtml(step.label || "Next step")}</strong><small>${escapeHtml(step.detail || "")}</small></span>
        </li>
      `).join("");
      const transparency = (Array.isArray(brief.transparency) ? brief.transparency : [])
        .map((item) => `<div><dt>${escapeHtml(item.label || "Detail")}</dt><dd title="${escapeHtml(item.value || "")}">${escapeHtml(item.value || "")}</dd></div>`)
        .join("");
      block.innerHTML = `
        <header class="agent-task-header">
          <span class="agent-task-mark"><span class="codicon codicon-sparkle" aria-hidden="true"></span></span>
          <div class="agent-task-heading"><strong>${escapeHtml(brief.title || "Task plan")}</strong><span>${escapeHtml(brief.summary || "")}</span></div>
          <span class="agent-task-state">Preparing</span>
        </header>
        <ol class="agent-task-steps">${rows}</ol>
        <div class="agent-task-live"><span class="agent-task-live-dot"></span><span>Preparing the run…</span></div>
        ${transparency ? `<details class="agent-task-transparency"><summary>Show run details</summary><dl>${transparency}</dl></details>` : ""}
      `;
      this.updateTaskStage("preflight");
      this.setLiveState({ kind: "planning", title: "Preparing the run", detail: brief.summary || "Choosing the next safe step.", meta: "PLAN" });
    },
    updateTaskStage(phase = "") {
      const block = this.taskBriefEl;
      if (!block) return;
      const rows = [...block.querySelectorAll("[data-task-step]")];
      if (!rows.length) return;
      const targetId = taskPhaseStepId(phase) || rows.find((row) => row.dataset.taskStatus === "active")?.dataset.taskStep || rows[0].dataset.taskStep;
      const targetIndex = Math.max(0, rows.findIndex((row) => row.dataset.taskStep === targetId));
      rows.forEach((row, index) => {
        const status = index < targetIndex ? "done" : index === targetIndex ? "active" : "pending";
        row.dataset.taskStatus = status;
        const icon = row.querySelector(".agent-task-step-icon");
        if (icon) icon.className = `agent-task-step-icon codicon ${taskStepIcon(status)}`;
      });
      const phaseLabel = String(phase || "Preparing").replace(/[-_]/g, " ");
      const live = block.querySelector(".agent-task-live span:last-child");
      if (live) live.textContent = `${phaseLabel[0]?.toUpperCase() || "P"}${phaseLabel.slice(1)} in progress`;
      const state = block.querySelector(".agent-task-state");
      if (state) state.textContent = "In progress";
    },
    noteTaskActivity(text, kind = "") {
      const live = this.taskBriefEl?.querySelector(".agent-task-live span:last-child");
      if (live && text) live.textContent = String(text).replace(/\s+/g, " ").trim().slice(0, 180);
      this.taskBriefEl?.classList.toggle("has-warning", kind === "warn");
    },
    completeTaskBrief(status = "complete") {
      const block = this.taskBriefEl;
      if (!block) return;
      this.turn.setAttribute("aria-busy", "false");
      block.dataset.taskStatus = status;
      block.classList.remove("is-active", "is-complete", "is-error", "is-inconclusive");
      block.classList.add(status === "complete" ? "is-complete" : status === "error" ? "is-error" : "is-inconclusive");
      const rows = [...block.querySelectorAll("[data-task-step]")];
      rows.forEach((row) => {
        if (status === "complete") row.dataset.taskStatus = "done";
        const icon = row.querySelector(".agent-task-step-icon");
        if (icon) icon.className = `agent-task-step-icon codicon ${taskStepIcon(row.dataset.taskStatus)}`;
      });
      const live = block.querySelector(".agent-task-live span:last-child");
      if (live) live.textContent = status === "complete" ? "Finished · result ready to review" : status === "error" ? "Stopped · review the error below" : "Finished with limitations · review the notes below";
      const state = block.querySelector(".agent-task-state");
      if (state) state.textContent = status === "complete" ? "Complete" : status === "error" ? "Stopped" : "Inconclusive";
      this.setLiveState({
        kind: status === "complete" ? "success" : status === "error" ? "error" : "verify",
        title: status === "complete" ? "Ready to review" : status === "error" ? "Run stopped" : "Review limitations",
        detail: status === "complete" ? "The requested work is complete." : status === "error" ? "The agent could not complete this task." : "The run finished without enough evidence for a complete result.",
        meta: status === "complete" ? "DONE" : status === "error" ? "STOPPED" : "REVIEW",
      });
    },
    ensureActivityLog() {
      if (this.activityLogEl) return this.activityLogEl;
      this.turn.classList.add("has-agent-run");
      const block = document.createElement("section");
      block.className = "agent-activity-log";
      block.innerHTML = `
      ...
      list.className = "agent-activity-lines";
      block.appendChild(list);
      this.turn.insertBefore(block, this.contentEl);
      this.activityLogEl = list;
      return list;
    },
    appendActivity(text, { kind = "info" } = {}) {
      const list = this.ensureActivityLog();
      const activityText = String(text || "").trim();
      const activityKey = `${kind}:${activityText}`;
      if (!activityText) return null;
      if (kind === "thinking" && this.reasoningActivityLine) {
        const message = this.reasoningActivityLine.querySelector(".agent-activity-message");
        if (message) message.textContent = activityText;
        this.lastActivityKey = activityKey;
        return this.reasoningActivityLine;
      }
      if (activityKey === this.lastActivityKey) return null;
      this.lastActivityKey = activityKey;
      const line = document.createElement("div");
      line.className = `agent-activity-line ${kind}`;
      line.innerHTML = `<span class="agent-activity-icon codicon ${activityIcon(kind)}" aria-hidden="true"></span><time>${escapeHtml(formatActivityTime())}</time><span class="agent-activity-message">${escapeHtml(activityText)}</span>`;
      list.appendChild(line);
      if (kind === "thinking") this.reasoningActivityLine = line;
    ...
    setStatus(text) {
      ...
      const statusKind = agentStateKindForText(statusText) === "thinking" ? "thinking" : "status";
      if (statusText) this.appendActivity(statusText, { kind: statusKind });
      if (text) {
        this.updateTaskStage(text);
        this.noteTaskActivity(text, "status");
        const kind = agentStateKindForText(text);
        const title = {
          thinking: "Thinking privately",
          planning: "Planning next step",
          working: "Agent working",
          decision: "Checking a decision",
          question: "Waiting for input",
          verify: "Verifying result",
          error: "Needs attention",
        }[kind] || "Agent working";
        this.setLiveState({ kind, title, detail: String(text), meta: kind === "decision" ? "CHECK" : kind === "verify" ? "VERIFY" : "LIVE" });
      }
    ...
    syncDisplay({ animateToken = "" } = {}) {
      ...
      if (text) {
        this.clearStatus();
        this.setLiveState({ kind: "working", title: "Writing response", detail: "Drafting the answer from the current evidence.", meta: "LIVE" });
    ...
    finalizeContent() {
      ...
      if (!this.taskBriefEl && !this.contentEl.hidden) {
        this.setLiveState({ kind: "success", title: "Ready", detail: "Response complete.", meta: "DONE" });
      }
      this.clearStatus();
    ...
    pruneIfEmpty() {
      ...
      const hasActivity = Boolean(this.activityLogEl?.childElementCount);
      const hasTools = this.turn.querySelector(".tool-card");
      if (!hasContent && !statusActive && !hasThinking && !hasActivity && !hasTools) {
        this.turn.remove();
      }
    },
    showPrivateReasoning() {
      if (activeThinkingPhase) return false;
      ...
      body.setAttribute("aria-label", "Private reasoning status");
      body.textContent = "Reasoning is running privately. Hidden chain-of-thought and system instructions stay out of the chat.";
      ...
      this.turn.classList.add("has-agent-run");
      this.setLiveState({ kind: "thinking", title: "Thinking privately", detail: "Working through the request without exposing hidden reasoning.", meta: "THINKING" });
      scrollMessages();
      return true;
    },
    finalizeThinking() {
      const reasoningCompleted = this.completeReasoningActivity();
      ...
      phase.body.textContent = "Reasoning completed privately. Raw chain-of-thought and system instructions were not displayed.";
    ...
    completeReasoningActivity() {
      const line = this.reasoningActivityLine;
      if (!line) return false;
      line.classList.remove("thinking");
      line.classList.add("success");
      const icon = line.querySelector(".agent-activity-icon");
      if (icon) icon.className = `agent-activity-icon codicon ${activityIcon("success")}`;
      const message = line.querySelector(".agent-activity-message");
      if (message) message.textContent = "Reasoning complete";
...
async function sendMessageWithAgentRuntime() {
  ...
  try {
    ...
    assistant.contentEl.classList.add("streaming");
    assistant.appendActivity(`Using ${selectedModel}`, { kind: "meta" });
    ...
    if (payload.type === "task_brief" && payload.brief) {
      ...
      assistant.appendActivity("Plan ready · starting the run", { kind: "meta" });
    ...
    if (payload.type === "activity") {
      assistant.appendActivity(payload.text, { kind: payload.kind || "info" });
      assistant.noteTaskActivity(payload.text, payload.kind || "info");
      if (payload.kind === "warn") setAgentStatus(payload.text || `${modeLabel()} warning`);
      return;
    }

    if (payload.type === "thinking") {
      assistant.showPrivateReasoning();
      assistant.appendActivity("Model is reasoning...", { kind: "thinking" });
      assistant.setLiveState({ kind: "thinking", title: "Thinking privately", detail: "Working through the request without exposing hidden reasoning.", meta: "THINKING" });
      return;
    }
    ...
    if (payload.type === "agent_terminal") {
      if (payload.phase === "start") {
        assistant.appendActivity(`Terminal command: ${payload.command || payload.toolName || "command"}`, { kind: "tool" });
        assistant.setLiveState({ kind: "working", title: "Agent working in terminal", detail: payload.command || payload.toolName || "Running a bounded command.", meta: "TERMINAL" });
        TerminalManager.attachAgentSession({
          id: payload.id,
    ...
    if (payload.type === "tool_result" && payload.tool && payload.result) {
      ...
      assistant.appendActivity(`Finished ${payload.tool.toolName || payload.tool.action}: ${String(summary).slice(0, 240)}`, {
        kind: payload.result.error ? "warn" : "success",
      });
      ...
      assistant.setLiveState({
        ...
        title: payload.result.error ? "Action needs attention" : "Action complete",
        detail: String(summary).slice(0, 180),
        meta: payload.result.error ? "FAILED" : "DONE",
      });
    ...
    if (result?.error) {
      ...
      if (!assistant.taskBriefEl) assistant.turn.remove();
      addErrorMessage(result.error);
      chatHistory.splice(historyStart);
      syncActiveChatSession();
      return;
    ...
    if (stopRequested) {
      ...
      assistant.completeTaskBrief("inconclusive");
      assistant.finalizeContent();
      updateContextUsage();
      return;
    }
    ...
    assistant?.completeTaskBrief?.("error");
    if (assistant && !assistant.taskBriefEl) assistant.turn?.remove();
    chatHistory.splice(historyStart);
