...
test("security workspace exposes Traffic Raw history with request and response details", () => {
  ...
  assert.ok(html.includes('src="bugbounty/security-inspector.js"'));
...
test("terminal sash resizing is frame-synchronized and deduplicates PTY dimensions", () => {
  const renderer = fs.readFileSync(path.join(__dirname, "..", "src", "renderer.js"), "utf8");
  const terminal = fs.readFileSync(path.join(__dirname, "..", "src", "terminal.js"), "utf8");
  const css = fs.readFileSync(path.join(__dirname, "..", "src", "style.css"), "utf8");
...
test("target workspace exposes Core, Context, Custom, Toolbox, and configurable commands", () => {
  const html = fs.readFileSync(path.join(__dirname, "..", "src", "index.html"), "utf8");
  const renderer = fs.readFileSync(path.join(__dirname, "..", "src", "renderer.js"), "utf8");
  ...
  assert.ok(html.includes('data-safety-mode="safe"'));
  ...
  assert.ok(html.includes('id="chat-safety-button"'));
  assert.ok(html.includes('role="switch"'));
  assert.ok(html.includes('chat-safety-switch-thumb'));
  assert.ok(html.includes('id="chat-safety-tooltip"'));
  ...
  assert.doesNotMatch(html, /data-action="create-project"/);
  assert.doesNotMatch(html, /data-action="open-project"/);
  assert.match(html, /data-action="create-assessment"[^>]*>Create Assessment Folder/);
  assert.match(html, /data-action="open-assessment"[^>]*>Open Existing Assessment/);
