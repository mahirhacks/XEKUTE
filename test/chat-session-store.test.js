...
const { createChatSessionStore } = require("../src/chat-session-store");
...
test("chat v2 persistence encrypts canonical messages and recovers its last backup", (t) => {
  ...
  const state = {
    ...
    sessions: [{ id: "chat-secure", title: "Secrets", history: [{ role: "user", content: "private-token" }], messagesHtml: "<b>must not persist</b>" }],
  };
