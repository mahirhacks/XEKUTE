...
test("context checkpointing is automatic and renderer-owned compaction is absent", () => {
  ...
  assert.match(renderer, /function boxHasToolUsage/);
  ...
  assert.doesNotMatch(renderer, /assets\/icons\/compress_icon\.svg/);
  ...
  assert.match(css, /#messages \.context-checkpoint-notice \{[\s\S]{0,220}font: 400 14px/);
