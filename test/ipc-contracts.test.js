...
test("renderer sends prompts without model schemas and main owns registry catalog", () => {
  ...
  assert.match(main, /const runtimeTools = selectedCatalog\.tools/);
...
test("active IPC handlers and preload methods include live channels and omit removed approval/chat APIs", () => {
  ...
  for (const channel of ["agent:run", "context:tier1Usage", "project:create", "chat-history:load", "chat-history:begin", "chat-history:event", "chat-history:delete", "chat-history:flush", "mcp:read", "mcp:ensure", "kali-access:get", "kali-access:save", "kali-access:test", "kali-access:pickIdentity", "settings:credentialsGet", "settings:credentialCreate", "settings:credentialSave", "settings:credentialDelete", "proxy:browserLaunch", "proxy:browserStatus"]) {
