...
test("AI and automated actions retain authority enforcement", () => {
  const renderer = read("src/ui/bootstrap.js");
  const policy = read("src/agent/policy/policy-engine.js");
...
test("operator HTTP, proxy, and Map IPC paths bypass only AI authority", () => {
  ...
  assert.match(map, /!operatorInitiated && settings\.authority/);
...
test("delete lifecycle serializes refreshes and always restores an idle composer", () => {
  ...
  assert.match(renderer, /function restoreChatComposerAfterUiAction[\s\S]*?if \(!chatInput \|\| streaming\) return;[\s\S]*?chatInput\.disabled = false;[\s\S]*?chatInput\.readOnly = false/);
});
