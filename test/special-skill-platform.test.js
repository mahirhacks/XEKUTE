...
test("internal Markdown skills support safe explicit invocation and remain subordinate to the canonical system prompt", () => {
  ...
  assert.match(resolved.prompt, /Project Memory[\s\S]*Investigation Memory[\s\S]*Evidence Memory/);
...
test("pentest is one Markdown skill with a shared-memory loop coordinator", () => {
  ...
  assert.match(loaded.resources[0].content, /Do not create a Pentest-specific run directory/);
  assert.match(loaded.resources[0].content, /block finalizer update Project and Investigation Memory/);
